...
function aw_check( field ) {
    ...
    jQuery(field + ' .input-box').each(function() {
        ...
        var checkfield = jQuery(this).attr('data-checkfield');
        var checkrule = jQuery(this).attr('data-checkerror');
...
function aw_check_init( field, autocheck ) {
    ...
    jQuery(field + ' .input-box').each(function() {
        ...
        var checkfield = jQuery( this ).attr('data-checkfield');
        var checklabel = jQuery( this ).attr('data-checklabel');
        ...
        if ( checkfield != '' && checkfield != undefined ) {
            ...
            if ( checklabel != '' && checklabel != undefined ) {
                ...
                // Insert value field
                jQuery( checkfield ).parents('.input-box').prepend( '<span class="label">' +  checklabel + '</span>' );
                
                // Set focus on field on label click
                jQuery( checkfield ).parents('.input-box').find( '.label' ).click( function() {
                    
                    checkfield = jQuery( this ).parents('.input-box').attr( 'data-checkfield' );
...
function aw_check_toggle_overlay_labels( referer ) {
    ...
    var checkfield = jQuery( referer ).parents('.input-box').attr( 'data-checkfield' );
    var label = jQuery( referer ).parents('.input-box').find( '.label' );
...
function aw_check_init_auto_check( referer ) {
    ...
    // Create all variables
    var checkparent = jQuery( referer ).parents('.input-box');
    var checkfield = jQuery( referer ).parents('.input-box').attr( 'data-checkfield' );
    var checkrule = jQuery( referer ).parents('.input-box').attr( 'data-checkrule' );
...
function aw_check_field( referer, checkpassed ) {
    ...
    var checkfield = jQuery( referer ).parents('.input-box').attr('data-checkfield');
    var checkrule = jQuery( referer ).parents('.input-box').attr('data-checkrule');
    var checkerror = jQuery( referer ).parents('.input-box').attr('data-checkerror');
